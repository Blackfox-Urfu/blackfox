...
# ### ИЗМЕНЕНИЕ №1: Адаптация под новый ключ "text_content" ###
def extract_text(message):
    """Извлекает текст из поля 'text_content' нового формата JSON."""
    # Раньше было: message.get("text", "")
    text_content = message.get("text_content", "")
    if isinstance(text_content, str):
        return text_content
    # Это предупреждение поможет отловить неожиданные типы данных
    print(f"Warning: Unexpected type for 'text_content' in message id {message.get('id')}: {type(text_content)}. Expected str.")
    return ""

# ### ИЗМЕНЕНИЕ №2: Адаптация полей для CSV под новый формат JSON ###
def extract_message_data(message):
    """Извлекает данные из сообщения для CSV, используя ключи нового формата."""
    extracted_text = extract_text(message)
    # Сообщения без текста пропускаем, как и раньше
    if not clean_text(extracted_text):
        return None

    # Адаптируем извлечение метаданных
    photo_path = ""
    ...
    return {
        ...
        "date": message.get("date_unixtime", ""), # Новый ключ
        "from": message.get("from_id", ""),       # Новый ключ
        "photo": photo_path,                    # Адаптированная логика
        "file_name": photo_path                 # Используем path из аттачмента
    }
# ### КОНЕЦ ИЗМЕНЕНИЙ ###
...
#
# --- Далее весь код остается БЕЗ ИЗМЕНЕНИЙ, ---
# так как он не зависит от формата исходного JSON.
# Я привожу его полностью для целостности.
#
    ...
    if os.path.exists(final_model_path):
         ...
         try:
             ...
             if len(np.unique(all_test_labels)) > 1 and len(all_test_positive_probs) > 0:
                 ...
                 plt.plot(thresholds_prc, f1_scores, label='F1-Score', lw=2, linestyle=':')
