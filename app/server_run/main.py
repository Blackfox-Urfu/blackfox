...
# Ограничиваем максимальное число пикселей (например, 100 Мп).
...
async def classify_message_endpoint(text: str = Form(""), image: Optional[UploadFile] = File(None)):
    if not multimodal_classifier.model: 
        raise HTTPException(status_code=503, detail="Multimodal ad model not available.")
    try:
        ...
        if image and image.filename:
            ...
            if contents: 
                ...
                try:
                    ...
                    logger.warning(f"Image Bomb detected! Filename: {image.filename}")
                    raise HTTPException(status_code=400, detail="Image too large (Decompression Bomb protection)")
        ...
        if not text.strip() and not pil_image: 
            return AdPredictionResponse(prediction_prob_ad=0.0, is_ad=False, confidence=1.0, error="No content")
        ...
        logger.error(f"Error in multimodal endpoint: {e}", exc_info=True)
